...
const PopUpSys = (props) => {
    ...
    const {deleteType,setDeleteType}=usePageContext();
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await axios.get(`${"http://192.168.73.243:6900/AppointmentAPI/GetAppointmentById"}/${appotmentId}`, {
                    headers: headers,
                });
  ...
  const handelEditPress=async()=>{
        console.log("Bearer "+token.replace(/"/g, ''));
                ...
                try {
                  // Display loading indicator or disable the login button here
                    const response = await axios.put(`http://192.168.73.243:6900/AppointmentAPI/UpdateAppointment/${appotmentId}`,requestBody, {
                    headers: headers,
                    });
                if (response.status === 200) {
                    setShowDelete(false);
    ...
    const handelDeletePress=async()=>{
        if(deleteType==true){
            ...
            console.log(appotmentId);
            try {

              // Display loading indicator or disable the login button here
                const response = await axios.put(`http://192.168.73.243:6900/AppointmentBookedAPI/CloseAppointmentBooked?appointmentBookedId=4018`, {
                headers: headers,
                });
            if (response.status === 200) {
                setShowDelete(false);
              ...
              // Hide loading indicator or enable the login button here
                ...
                try {
                  // Display loading indicator or disable the login button here
                    const response = await axios.delete("http://192.168.73.243:6900/AppointmentAPI"+`?appointmentId=${appotmentId}`, {
                    headers: headers,
                    });
                if (response.status === 200) {
                    setShowDelete(false);

...
const PostDetels = ({  navigation ,route  }) => {
    ...
    const handelAcceptPress= async()=>{
        ...
        const headers = {
            ...
            try {
                ...
                `${"http://192.168.73.243:6900/AppointmentBookedAPI/AppointmentBooked Accepted"}?appointmentBookedId=${props.id.toString()}`,
    ...
    const  handelRejectPress= async()=>{
        console.log("Bearer "+token.replace(/"/g, ''));
                ...
                try {
                  // Display loading indicator or disable the login button here
                    const response = await axios.delete("http://192.168.73.243:6900/AppointmentBookedAPI/AppointmentBooked Rejected"+`?appointmentBookedId=${props.id}`, {
                    headers: headers,
                    });
                if (response.status === 200) {
                    setchangeAprvelApp((prev)=>{return !prev})
